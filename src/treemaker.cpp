...
public:
  ...
  static Vector2* const get_ptr() {
    if (lazy_arena::mem == nullptr) lazy_arena::mem = (Vector2*)malloc(sizeof(Vector2) * get_count());
...
Vector2* lazy_arena::mem = nullptr;
...
void draw_branching(Vector2 root, float lastangle, int depth, float decay,
  ...
  // Draw line.
  DrawSplineBezierCubic(lazy_arena::get_ptr(), lazy_arena::get_count(), thick, color);
  //DrawSplineSegmentLinear(root, end, thick, color);
  // DrawText(TextFormat("d%i_a%.0f_l%.0f_t%.0f", depth, angle, length, thick),
  // end.x, end.y, 8, WHITE);
  ...
  if (floatrand(rand) < decay)
    // Sibling branches.
    draw_branching(root, lastangle, depth, decay * 0.8, (mt19937)rand());
  if (floatrand(rand) < decay)
    // Nested branches.
    draw_branching(end, angle, depth + 1, decay * 0.8, (mt19937)rand());
  else
...
void draw_leaves(Vector2 root, mt19937 rand) {
  ...
  for (int i = 0; i < SUBCIRC_COUNT; i++)
    DrawCircleV(
        Vector2Add(root, {Lerp(-BGCIRC_SIZE, BGCIRC_SIZE, floatrand(rand)),
...
void draw_indicator(int posx, int posy) {
  ...
  DrawLine(posx + INDICATOR_RADIUS, posy, posx - INDICATOR_RADIUS, posy, BLACK);
  DrawLine(posx, posy + INDICATOR_RADIUS, posx, posy - INDICATOR_RADIUS, BLACK);
}